...
from math_datasets.generators import generate_responses, ReWOOGenerateV2 as ReWOOGenerate
...
RESULTS_DIR = Path(__file__).parent.parent.parent / "results" / "rewoo_v2"
...
def generate_responses_for_gemini_models(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test"):
    for model_name in model_names:
        ...
        for dataset in datasets:
            ...
            df = evaluate_detail(get_google_name_identifer(model_name), dataset=dataset, use_transformated_answers=False, additional_metrics=ReWOOGenerate, save_dir=SAVE_DIR.as_posix(), use_first_n=first_n)
...
def generate_responses_for_ollama_models(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test"):
    for model_name in model_names:
        generator = ReWOOGenerate.init_ollama(model_name=model_name, with_examples=True)
        for dataset in datasets:
            generate_responses(
                dataset, 
...
def generate_responses_for_local_models_before_fine_tuning(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test"):
    for model_name in model_names:
        ...
        for dataset in datasets:
            ...
            f = RESULTS_DIR / dataset.name / f"{get_model_name_identifer(model_name, fine_tuned=False)}.xlsx"
            f.parent.mkdir(parents=True, exist_ok=True)
            df.to_excel(f, index=False)
            dataset.clear_cache()
