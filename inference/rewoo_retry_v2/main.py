...
from math_datasets.generators import generate_responses, ReWOORetryV2Generate
...
RESULTS_DIR = Path(__file__).parent.parent.parent / "results" / "rewoo_retry_v2"
...
def generate_responses_for_gemini_models(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test"):
    for model_name in model_names:
        generator = ReWOORetryV2Generate.init_gemini(model_name=model_name, sleep_time=15)
        for dataset in datasets:
            ...
            df = evaluate_detail(get_google_name_identifer(model_name), dataset=dataset, use_transformated_answers=False, additional_metrics=ReWOORetryV2Generate, save_dir=SAVE_DIR.as_posix(), use_first_n=first_n)
...
def generate_responses_for_ollama_models(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test"):
    for model_name in model_names:
        generator = ReWOORetryV2Generate.init_ollama(model_name=model_name, with_examples=True)
        for dataset in datasets:
            generate_responses(
                dataset, 
            ...
            df = evaluate_detail(get_ollama_model_name_identifer(model_name), dataset=dataset, use_transformated_answers=False, additional_metrics=ReWOORetryV2Generate, save_dir=SAVE_DIR.as_posix(), use_first_n=first_n)
