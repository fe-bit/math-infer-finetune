...
def evaluate(model_name: str, output_dir: str, datasets: list[Dataset], checkpoint_dir: str, first_n: int=100, with_peft: bool=True, with_quantization: bool=False, untrained_only: bool=False):
    eval_name_before_training = model_name + "/before_training"
    eval_name_after_training = model_name + "/after_training"

    evaluate_one(
        generator=TransformersGenerate(TransformerLLM(model_name, dtype=torch.bfloat16, quantization=with_quantization)), 
        datasets=datasets, 
        first_n=first_n, 
    ...
    if not untrained_only:
        if with_peft:
            llm = TransformerLLM(model_name=model_name, dtype=torch.bfloat16, quantization=with_quantization)
...
if __name__:
    ...
    parser.add_argument("--quantized", action="store_true", default=False,
                       help="Whether to use Quantization for LLM")
    parser.add_argument("--untrained-only", action="store_true", default=False,
                       help="Whether to only evaluate the untrained model")
