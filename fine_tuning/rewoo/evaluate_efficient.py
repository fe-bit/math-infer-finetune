...
import argparse
from rewoo_local_efficient import ReWOOLocalModel
import time
...
def evaluate(model_name: str, output_dir: str, datasets: list[Dataset], checkpoint_dir: str, first_n: int=100, with_peft: bool=True, resume_trained: bool=False):
    ...
    evaluate_one(
        generator=ReWOOModelGenerate(ReWOOLocalModel(llm=llm, with_examples=True)), 
        datasets=datasets, 
        first_n=first_n, 
    ...
    if with_peft:
        ...
        evaluate_one(
            generator=ReWOOModelGenerate(ReWOOLocalModel(llm=generator, with_examples=False)), 
            datasets=datasets, 
            first_n=first_n, 
            model_name=eval_name_after_training,
            save_dir=output_dir,
            overwrite=not resume_trained
        )
        del llm
    else:
