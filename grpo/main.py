...
def generate_responses_for_local_models_after_fine_tuning(datasets: List[Dataset], model_names: List[str], first_n: int|None=None, dataset_split: Literal["test", "train"]="test", resume: bool=False, with_peft: bool = True):
    for model_name in model_names:
        ...
        if with_peft:
            ...
            llm = TransformerLLM(
                model_name=get_checkpoint_path(model_name=model_name).as_posix(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with-peft", action="store_true", default=False,
                       help="Whether to use PEFT for fine tuned models")
    ...
    if args.model_name is None:
        ...
        generate_responses_for_local_models_after_fine_tuning(datasets, models, first_n=first_n, dataset_split="test", resume=args.resume, with_peft=args.with_peft)
        ...
        print(df)
        filepath = SAVE_DIR / f"{args.model_name}_dp_evaluation.csv"
